...
struct ContentView: View {
    ...
    @State private var tabletSelectedTab: Int = 0
    @State private var tabletSelectedDeckPreview: SavedDeckState? = nil
    @State private var tabletViewSize: CGSize = .zero
    ...
    private var isTabletLandscape: Bool {
        guard horizontalSizeClass == .regular else { return false }
        let w = tabletViewSize.width
        let h = tabletViewSize.height
        ...
        contentBody
            .background(
                GeometryReader { geo in
                    Color.clear
                        .onAppear { tabletViewSize = geo.size }
                        .onChange(of: geo.size) { tabletViewSize = $0 }
                }
            )
    }
    ...
    // MARK: - Tablet landscape layout

    private var tabletLandscapeView: some View {
        VStack(spacing: 0) {
            // Top tab bar
            HStack(spacing: 0) {
                let tabs = ["카드검색", "덱빌더", "덱목록"]
                ForEach(tabs.indices, id: \.self) { idx in
                    Button(action: { tabletSelectedTab = idx }) {
                        VStack(spacing: 2) {
                            Text(tabs[idx])
                                .fontWeight(tabletSelectedTab == idx ? .bold : .regular)
                                .foregroundColor(tabletSelectedTab == idx ? .primary : .secondary)
                                .padding(.horizontal, 16)
                                .padding(.top, 10)
                                .padding(.bottom, 6)
                            Rectangle()
                                .fill(tabletSelectedTab == idx ? Color.accentColor : Color.clear)
                                .frame(height: 2)
                        }
                    }
                    .buttonStyle(.plain)
            ...
            Group {
                if tabletSelectedTab == 0 {
                    tabletCardSearchTab
                } else if tabletSelectedTab == 1 {
                    tabletDeckBuilderTab
                } else {
                    tabletDeckListTab
                }
            }
        ...
        .onChange(of: viewModel.state.detailKoText) { _ in refreshDetailLines() }
        .onChange(of: viewModel.state.detailJaText) { _ in refreshDetailLines() }
        .onChange(of: preferredLanguageRawValue) { _ in refreshDetailLines() }
        .onChange(of: multiWordTags) { tags in
            rebuildTagRegexCache(tags)
            refreshDetailLines()
        }
        .sheet(isPresented: Binding(get: { showingMenu }, set: { showingMenu = $0 })) {
            MenuSheet(
                state: viewModel.state,
                themeMode: selectedThemeModeBinding,
                preferredLanguage: selectedPreferredLanguageBinding,
        ...
        .alert(
            ...
            isPresented: Binding(
                get: { viewModel.state.updateDialog != nil },
                set: { newValue in if !newValue { viewModel.onUpdateDialogDismiss() } }
            ),
            presenting: viewModel.state.updateDialog,
        ) { _ in
            Button("나중에", role: .cancel) { viewModel.onUpdateDialogDismiss() }
            Button("업데이트") { viewModel.onUpdateDialogConfirm() }
        } message: { dialog in
            Text(
                "DB 업데이트가 있습니다. 업데이트 하시겠습니까?\n로컬 DB 날짜: \(dialog.localDate ?? "없음")\nGitHub DB 날짜: \(dialog.remoteDate)\(dialog.remoteDigest.map { "\nGitHub DB 식별자: \($0.prefix(8))" } ?? "")"
            )
        ...
        .alert(
            ...
            isPresented: Binding(
                get: { renamingDeckID != nil },
                set: { newValue in if !newValue { renamingDeckID = nil } }
            )
        ) {
            TextField("덱 이름", text: $renamingDeckTitle)
            Button("취소", role: .cancel) { renamingDeckID = nil }
            Button("저장") { commitRenameDeck() }
        } message: {
            Text("변경할 덱 이름을 입력하세요.")
        }
    ...
    // Tab 0 — 카드검색 master-detail
    private var tabletCardSearchTab: some View {
        HStack(spacing: 0) {
            ...
            .frame(width: tabletViewSize.width > 0 ? tabletViewSize.width * 0.38 : 390)
    ...
    // Tab 1 — 덱빌더 master-detail
    private var tabletDeckBuilderTab: some View {
        HStack(spacing: 0) {
            ...
            .task(id: tabletSelectedTab) {
                guard tabletSelectedTab == 1 else { return }
                        ...
                        VStack(spacing: 8) {
                            ForEach(deckEntries.indices, id: \.self) { i in
                                HStack {
                                    ...
                                    Button("-") {
                                        deckEntries[i].qty -= 1
                                        if deckEntries[i].qty <= 0 { deckEntries.remove(at: i) }
                                    }
                                    Button("+") {
    ...
    // Tab 2 — 덱목록 master-detail
    private var tabletDeckListTab: some View {
        HStack(spacing: 0) {
                ...
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 0) {
                        ForEach(savedDecks) { deck in
                            Button {
                                tabletSelectedDeckPreview = deck
                            } label: {
                                HStack {
                                    VStack(alignment: .leading, spacing: 4) {
                                        Text(deck.title).bold()
                                        Text("카드 \(deck.entries.map(\.qty).reduce(0, +))장")
                                            .font(.caption)
                                            .foregroundColor(.secondary)
                                    }
                                    Spacer()
                                    if tabletSelectedDeckPreview?.id == deck.id {
                                ...
                                .padding(.horizontal, 12)
                                .padding(.vertical, 10)
                                .background(tabletSelectedDeckPreview?.id == deck.id ? Color.accentColor.opacity(0.1) : Color.clear)
                            }
                            .buttonStyle(.plain)
                            Divider()
                        }
            ...
            if let deck = tabletSelectedDeckPreview {
                VStack(alignment: .leading, spacing: 12) {
                    HStack {
                        Text(deck.title)
                        ...
                        Menu {
                            ...
                            Menu("코드로 내보내기") {
                                Button("홀로듀얼 코드") { exportHoloDuelCodeToClipboard(deck) }
                                Button("홀로델타 .json 파일") { exportHoloDeltaCodeToClipboard(deck) }
                                Button("부시나비 코드") { exportBushiroadCodeToClipboard(deck) }
                            }
                            Button("이미지로 내보내기") { exportDeckImage(deck) }
                            Button("삭제", role: .destructive) {
                                removeDeck(deck.id)
                                tabletSelectedDeckPreview = nil
                            }
                        } label: {
                    ...
                    HStack {
                        Text("오시 \(deck.entries.filter { isOshi($0.card) }.map(\.qty).reduce(0, +))/1")
                        Text("옐 \(deck.entries.filter { isYell($0.card) }.map(\.qty).reduce(0, +))/20")
                        Text("덱 \(deck.entries.filter { !isOshi($0.card) && !isYell($0.card) }.map(\.qty).reduce(0, +))/50")
                        Text("합계 \(deck.entries.map(\.qty).reduce(0, +))장")
                        Spacer()
                    }
                    .font(.footnote)
                    .foregroundColor(.secondary)
                        ...
                        LazyVStack(alignment: .leading, spacing: 8) {
                            ForEach(deck.entries) { entry in
                                HStack {
                                    ...
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text("\(entry.card.cardNumber) | \((entry.card.nameKo.isEmpty ? entry.card.nameJa : entry.card.nameKo))")
                                            .lineLimit(1)
                                        let r = entry.displayRarity
                                        if !r.isEmpty {
                                            Text(r).font(.caption2).foregroundColor(.secondary)
                                        }
                                    }
        ...
        .task(id: tabletSelectedTab) {
            guard tabletSelectedTab == 2 else { return }
            await loadSavedDecks()
        }
    }
